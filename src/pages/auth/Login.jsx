...
export default function Login() { 
  ...
  async function handleSignIn(e){
    ...
    console.log(email, password, DASHBOARD)
    // setLoading(false);
  };
  ...
  return (
    <div className='w-full h-screen flex flex-col items-center justify-center bg-light'>
      ...
      >
        <h1 className='text-h5 font-bold font-head text-center'>Welcome back!</h1>
        ...
        <form onSubmit={handleSignIn} className='mt-8 min-w-[80%]'>
          ...
          <div className='relative'>
            <input
              ...
              className={
                `block ${errors.email === null ? 'mb-4' : 'mb-0' } pt-2 pb-2 pr-8 pl-2 w-full rounded-[64px] border-2 border-base focus:outline-none focus:ring focus:border-light placeholder:text-gray-50`
              }
          ...
          errors.email !== null &&
          (
            <div className='mt-1 text-small text-center mb-4 text-red-500'>{errors.email}</div>
          )
          }
          ...
          {errors.password !== null && (
            <div className='mt-1 text-small text-center mb-4 text-red-500'>{errors.password}</div>
          )}

          <button 
            className={`w-full ${isLoginLoading ? 'opacity-50' : 'opacity-100'} bg-base text-white rounded-[64px] pt-2 pb-2 mb-8 relative`}
