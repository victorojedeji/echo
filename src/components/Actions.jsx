...
export default function Actions({post}) {
  ...
  const {toggleLike, likeLoading} = useToggleLike({id, isLiked, uid: user?.id});
    ...
    <div className='border-t-2 mt-8 p-2 flex items-center justify-around bg-white rounded-[8px]'>
       ...
       ? <div className='flex gap-1 items-center'>
          <div 
            ...
            onClick={toggleLike}
          >
            {isLiked ? <AiFillHeart className='text-red-600' /> : <AiOutlineHeart  className='text-red-600'/>}
          </div>
       ...
       <Link to={`${AUTH}/comments/${id}`}>
        <div className='flex gap-1 items-center'>
            <div className='hover:bg-blue-100 p-2 rounded-[50%] ease-in duration-100'>
              {/* <FaRegCommentDots /> */}
