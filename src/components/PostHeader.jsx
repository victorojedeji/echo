...
export default function PostHeader({post}) {
  ...
  return (
    <div className='border-b-2 flex justify-between items-center relative'>
      ...
      <button className='text-para w-[64px] flex justify-end items-center' onClick={handleOption}>
        <BsThreeDots />
      </button>
      ...
      {toggleOptions && ( 
        <div className='absolute right-0 top-10 bg-white p-4 rounded-[8px]'>
          {!deleteLoading 
          ? <button className='flex gap-2 items-center pt-2 mb-2' onClick={deletePost} >
              <FiDelete className='text-red-600 text-small' /><span className='text-small text-red-600'>Delete</span>
            </button>
          : <div>
